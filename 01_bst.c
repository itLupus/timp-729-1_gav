...
int main()
{
    ...
    init(t);
    long int value;
    // 1. Считывает 4 элемента и заносит их в дерево
    ...
    // 3. Считывает 3 элемента и заносит их в деревео
    // 4. Выводит дерево
    ...
    // 5. Считывает m1 и ищет элемент с данным значением в дереве
    // вывести значение предка, потомка1, потомка2
    // если предка или потомка нет, то _
    // если нет такого значения то -
    // пустая строка
    long int m1;
    scanf("%ld", &m1);
    node *foundNode;
    ...
    if (foundNode)
    {
        if (foundNode->parent && foundNode->parent != foundNode)
            printf("%ld ", foundNode->parent->value);
        else
            printf("_ ");
        if (foundNode->left)
            printf("%ld ", foundNode->left->value);
        else
            printf("_ ");
        if (foundNode->right)
            printf("%ld", foundNode->right->value);
        else
            printf("_");
    }
    else
    ...
    // 6. считывает m2 и ищет элемент с заданным значением в дереве
    // выводит через пробел значение предка
    scanf("%ld", &m1);
    foundNode = find(t, m1);
    if (foundNode)
    {
    ...
    node *tempParent = (node *)malloc(sizeof(node));
    // 7. Считывает m3 и удаляет из дерева элемент с заданным значением
    scanf("%ld", &m1);
    int checkRemove = remove_node(t, m1);
    if (checkRemove == 0)
        t->count = t->count - 1;
    if (t->count > 0)
    {
        tempParent = t->root;
        t->root = tempParent->parent;
    }

    print_tree(t);

    // 9. выполняет левый поворот дерева, относительно корня, пока это возможно
    node *rotateNode = t->root;
    while (rotate_left(rotateNode) == 0)
    {
        t->root = rotateNode->parent;
        rotateNode = t->root;
    }
    ...
    node *rotateNode1 = rotateNode1 = t->root;
    while (rotate_right(rotateNode1) == 0)
    {
        t->root = rotateNode1->parent;
        rotateNode1 = t->root;
    }
